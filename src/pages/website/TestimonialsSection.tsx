...
const TestimonialsSection = () => {
  ...
  return (
    <section id="testimonials" ref={sectionRef} className="section-padding bg-gym-black">
      <div className="gym-container">
        ...
        <div className="relative">
          <div className="absolute -left-4 top-1/2 transform -translate-y-1/2 z-10">
            <button
              onClick={scrollLeft}
              className="bg-gym-yellow text-gym-black p-2 rounded-full"
            >
              <ChevronLeft size={24} />
            </button>
          </div>
          ...
          >
            {testimonials.map((testimonial, index) => (
              <div
                key={index}
                className="min-w-[300px] md:min-w-[400px] bg-gym-gray-800 rounded-lg p-6 shadow-lg snap-start"
              >
                    ...
                    <div className="flex items-center mt-1">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          size={16}
                          fill={i < testimonial.stars ? "#FFD100" : "none"}
                          className={i < testimonial.stars ? "text-gym-yellow" : "text-gray-500"}
                        />
                      ))}
                    </div>
                  </div>
