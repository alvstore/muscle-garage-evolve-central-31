...
export const membershipService = {
  ...
  /**
   * Register a member in all available biometric devices for a branch
   * @param member - The member object to register
   * @param branchId - The branch ID where the devices are located
   * @returns Promise with registration result
   */
  registerInBiometricDevices: async (member: any, branchId: string) => {
    try {
      if (!member || !member.id || !branchId) {
        console.error('Missing required parameters for biometric registration');
        return {
          success: false,
          message: 'Missing required parameters for biometric registration'
        };
      }

      // Check which biometric devices are available for this branch
      const deviceStatus = await import('@/services/biometricService')
        .then(module => module.checkBiometricDeviceStatus(branchId));

      const results = {
        success: false,
        hikvision: { success: false, message: 'Not configured' },
        essl: { success: false, message: 'Not configured' }
      };

      // Register in Hikvision if available
      if (deviceStatus.hikvision?.configured) {
        const hikvisionResult = await import('@/services/biometricService')
          .then(module => module.registerMemberInBiometricDevice({
            memberId: member.id,
        ...
        results.hikvision = hikvisionResult;
      }

      // Register in ESSL if available
      if (deviceStatus.essl?.configured) {
        const esslResult = await import('@/services/biometricService')
          .then(module => module.registerMemberInBiometricDevice({
            memberId: member.id,
            name: member.name || member.full_name,
            phone: member.phone || '',
        ...
        results.essl = esslResult;
      }

      // Consider the registration successful if at least one device registration succeeded
      results.success = results.hikvision.success || results.essl.success;

      return results;
    } catch (err) {
      console.error('Error in registerInBiometricDevices:', err);
      return {
        success: false,
        message: `Error registering in biometric devices: ${err instanceof Error ? err.message : String(err)}`
      };
    }
  }
};
