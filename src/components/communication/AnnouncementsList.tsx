  ...
  return <Card>
      ...
      <CardContent>
        {loading ? <div className="h-80 flex items-center justify-center">
          ...
          </div> : announcements.length === 0 ? <div className="text-center py-10">
            ...
            <Table>
              ...
              <TableBody>
                {announcements.map(announcement => <TableRow key={announcement.id}>
                    <TableCell className="font-medium">{announcement.title}</TableCell>
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        {announcement.targetRoles.map(role => <Badge key={role} variant="outline" className="capitalize">
                            {role}
                          </Badge>)}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex flex-wrap gap-1">
                        {announcement.channels.map(channel => <Badge key={channel} variant="secondary" className="capitalize">
                            {channel}
                          </Badge>)}
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      {announcement.expiresAt ? format(parseISO(announcement.expiresAt), "MMM dd, yyyy") : "Never"}
                    </TableCell>
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => handleDelete(announcement.id)} className="text-destructive focus:text-destructive">
