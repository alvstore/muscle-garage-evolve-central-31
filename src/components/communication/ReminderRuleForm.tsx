...
const ReminderRuleForm: React.FC<ReminderRuleFormProps> = ({
  ...
  useEffect(() => {
    if (editRule) {
      ...
      setTriggerType(editRule.trigger_type || editRule.triggerType || '');
      setTriggerValue(editRule.trigger_value || editRule.triggerValue || 7);
      setMessage(editRule.message || '');
      setTargetType('all_members'); // Default value as it may not exist in DB
      setActive(editRule.is_active || editRule.isActive || editRule.active || false);
      setChannels(editRule.send_via || editRule.sendVia || editRule.channels || ['app']);
      setTargetRoles(editRule.target_roles || editRule.targetRoles || ['member']);
    }
  }, [editRule]);
