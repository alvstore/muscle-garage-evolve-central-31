...
const Sidebar = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    adminNavSections.forEach(section => {
      const hasActiveItem = section.items.some(item => activeMenuItem === item.href || activeMenuItem.startsWith(item.href) && (activeMenuItem.length === item.href.length || activeMenuItem[item.href.length] === '/'));
      if (hasActiveItem) {
  ...
  const isActive = (path: string) => {
    ...
    // For child paths
    if (path !== '/' && activeMenuItem.startsWith(path) && (activeMenuItem.length === path.length || activeMenuItem[path.length] === '/')) {
      return true;
    }
    return false;
  };
  ...
  const sidebarBgColor = settings.semiDark ? isDark ? 'bg-[#283046] border-r border-[#3b4253]' // Dark mode with semi-dark enabled - matching Vuexy
  : 'bg-[#283046] border-r border-[#3b4253]' // Light mode with semi-dark enabled (dark sidebar)
  : isDark ? 'bg-[#283046] border-r border-[#3b4253]' // Dark mode with semi-dark disabled - matching Vuexy
  ...
  const textColor = settings.semiDark ? isDark ? 'text-foreground' // Dark mode with semi-dark enabled
  ...
  return <aside className={cn("fixed left-0 top-0 z-20 h-full flex flex-col transition-all duration-300", collapsed ? 'w-20' : 'w-64', sidebarBgColor, textColor)} data-sidebar="root">
      ...
      <div className={cn("flex items-center justify-between h-16 px-4 border-b", settings.semiDark ? isDark ? "border-border" : "border-[#3a4a76]" : "border-border")}>
        <Link to="/" className="flex items-center">
          {!collapsed ? <div className="flex items-center">
              ...
              <span className="text-gray-500 text-center font-bold px-0 text-lg mx-[7px]">Muscle Garaage</span>
            </div> : <svg width="32" height="22" viewBox="0 0 55 36" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-primary-500">
              <path d="M9.41568 35.4771C4.72529 35.4771 1.00098 31.7528 1.00098 27.0624V9.41485C1.00098 4.72446 4.72529 1.00015 9.41568 1.00015H45.5839C50.2743 1.00015 53.9986 4.72446 53.9986 9.41485V27.0624C53.9986 31.7528 50.2743 35.4771 45.5839 35.4771H9.41568Z" fill="currentColor" stroke={settings.semiDark ? isDark ? "white" : "black" : "white"} strokeWidth="1.5" />
              <path d="M25.0723 13.1176L25.0723 22.3137" stroke={settings.semiDark ? isDark ? "white" : "black" : "white"} strokeWidth="1.5" strokeLinecap="round" />
              <path d="M30.9248 13.1176L30.9248 22.3137" stroke={settings.semiDark ? isDark ? "white" : "black" : "white"} strokeWidth="1.5" strokeLinecap="round" />
              <path d="M25.0723 13.1176C25.0723 13.1176 25.0723 9.41485 27.9986 9.41485C30.9248 9.41485 30.9248 13.1176 30.9248 13.1176" stroke={settings.semiDark ? isDark ? "white" : "black" : "white"} strokeWidth="1.5" strokeLinecap="round" />
        ...
        <button onClick={toggleCollapse} className={cn("p-1 rounded-md", settings.semiDark ? "hover:bg-muted text-muted-foreground" : "hover:bg-[#3a4a76] text-gray-300")}>
              ...
              <div className={cn("flex items-center justify-between rounded-md px-3 py-2 cursor-pointer", settings.semiDark ? isDark ? "bg-[#343d55]" : "bg-[#343d55]" : "bg-muted")}>
                <div className="flex items-center">
                  ...
                  <div className="flex flex-col">
                    <span className={cn("text-xs font-medium", settings.semiDark ? "text-gray-400" : "text-gray-500")}>{currentBranch?.name?.split(' ')[0] || 'Muscle'}</span>
                    <span className={cn("text-sm font-semibold", settings.semiDark ? "text-gray-200" : "text-gray-700")}>{currentBranch?.name?.split(' ').slice(1).join(' ') || 'Garage Motera'}</span>
                  </div>
                </div>
                <div className="flex items-center">
      ...
      <div className="flex-1 overflow-y-auto py-2">
        <ul className="space-y-0.5 px-3">
          ...
          const activeBgColor = settings.semiDark ? isDark ? 'bg-muted' : 'bg-[#394b79]' : isDark ? 'bg-muted' : 'bg-gray-100';
          const hoverBgColor = settings.semiDark ? isDark ? 'hover:bg-muted' : 'hover:bg-[#394b79]' : isDark ? 'hover:bg-muted' : 'hover:bg-gray-100';
          const navTextColor = settings.semiDark ? isDark ? 'text-muted-foreground' : 'text-gray-200' : isDark ? 'text-muted-foreground' : 'text-gray-700';
          return <li key={sectionIndex} className="mb-1">
                <div className="flex items-center justify-between">
                  <button onClick={() => toggleSection(section.name)} className={cn("flex flex-1 items-center px-4 py-2.5 rounded-md transition-colors duration-200", hasActiveItem ? activeBgColor : hoverBgColor)}>
                    <span className={cn("mr-2.5 text-[1.25rem] opacity-70", navTextColor)}>
                      {section.items[0].icon}
                    </span>
                    {!collapsed && <span className={cn("flex-1 text-[0.875rem] font-normal text-left", navTextColor)}>
                        {section.name}
                      </span>}
                  </button>
                  
                  {!collapsed && <button onClick={() => toggleSection(section.name)} className={cn("p-1 rounded-md ml-1", hoverBgColor, navTextColor)}>
                      <ChevronDown size={16} className={`transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`} />
                    </button>}
                </div>
                
                {/* Section items */}
                {!collapsed && isExpanded && <ul className="mt-0.5 ml-7 space-y-0.5">
                    {section.items.map((item, itemIndex) => {
                // Skip items user doesn't have permission for
                if (!hasPermission(item.permission)) return null;
                const isItemActive = isActive(item.href);
                return <li key={itemIndex}>
                          <Link to={item.href} onClick={() => setActiveMenuItem(item.href)} className={cn("flex items-center px-4 py-2 rounded-md transition-colors duration-200", isItemActive ? 'bg-primary-500 text-white font-medium' : settings.semiDark ? isDark ? 'text-muted-foreground hover:bg-muted' : 'text-gray-200 hover:bg-[#394b79]' : isDark ? 'text-muted-foreground hover:bg-muted' : 'text-gray-700 hover:bg-gray-100')}>
      ...
      <div className="p-4 border-t border-[#3a4a76]">
        <div className="flex items-center justify-between">
          <button onClick={handleLogout} className="flex items-center text-gray-300 hover:text-primary-500" disabled={isLoggingOut}>
